...
CREATE TABLE if not exists public.watering_parameters
(
...
CREATE TABLE if not exists public.grow_programs
(
    id bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1 ),
    co2_id bigint,
...
-- --------------favorite_programs_assignments--------------- --

CREATE TABLE if not exists public.favorite_programs_assignments
(
    grow_program_id bigint NOT NULL,
    user_id bigint NOT NULL,
    CONSTRAINT favorite_programs_assignments_pkey PRIMARY KEY (grow_program_id, user_id),
    CONSTRAINT fkhvawi3rpvie0407jway58iw5i FOREIGN KEY (user_id)
        REFERENCES public.users (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fko0j7otpt6herfdsq2u1x9n5ru FOREIGN KEY (grow_program_id)
...
ALTER TABLE public.favorite_programs_assignments
    OWNER to postgres;

-- --------------greenboxes--------------- --

CREATE TABLE if not exists public.greenboxes
(
    id bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1 ),
...
('first temperature', '{18.1, 19.0, 21.0, 22.0, 23.0, 24.0, 24.0, 24.0, 24.0, 24.0, 24.0, 24.0, 24.0, 24.0, 23.0, 22.0, 24.0, 20.0, 19.0, 18.0, 18.0, 18.0, 18.0, 18.0}'),
...
INSERT INTO grow_programs (name, description, watering_parameters_id, temperature_id, light_id, co2_id) VALUES 
('drown potatoes', 'its to much water', 2, 1, 2, 1),
('drip irrigation', 'good choice', 1, 2, 1, 2),
('wasteland', 'its a desert', 3, 2, 2, 2);

INSERT INTO FAVORITE_PROGRAMS_ASSIGNMENTS (user_id, grow_program_id) VALUES
(1,1), (1,2),
(2,3);
